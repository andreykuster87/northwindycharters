...
export function DocEditor({ docDef, meta, verified, onSave }: Props) {
  ...
  return (
    <div className={`rounded-2xl border-2 overflow-hidden transition-all ${
      !hasData ? 'border-dashed border-amber-200 bg-amber-50' :
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex-1 min-w-0">
          ...
          {badge && <span className={`inline-block mt-1 text-[9px] font-black px-2 py-0.5 rounded-full border ${badge.cls}`}>{badge.label}</span>}
          {!hasData && <span className="inline-block mt-1 text-[9px] font-black px-2 py-0.5 rounded-full bg-amber-200 text-amber-800">⚠ Documento em falta</span>}
        </div>
        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
          ...
          {editable && !editing && (
            <button type="button"
              ...
              className="bg-blue-900 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-1 hover:bg-blue-800 transition-all">
          ...
          {editing && (
            <>
              <button type="button" onClick={handleSave}
                className="bg-green-600 text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-1 hover:bg-green-500 transition-all">
      ...
      {editing && (
        <div className="px-4 pb-4 space-y-3 border-t-2 border-white/60 pt-3">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[9px] font-black text-gray-500 uppercase block mb-1">Nº Documento</label>
              <input value={editNr} onChange={e => setEditNr(e.target.value.toUpperCase())} placeholder="Nº"
                className="w-full bg-white border-2 border-gray-200 rounded-xl py-2 px-3 text-xs font-bold text-blue-900 focus:border-blue-900 outline-none uppercase" />
            </div>
            <div>
              <label className="text-[9px] font-black text-gray-500 uppercase block mb-1">Validade</label>
              <input type="date" value={editVal} onChange={e => setEditVal(e.target.value)}
                className="w-full bg-white border-2 border-gray-200 rounded-xl py-2 px-3 text-xs font-bold text-blue-900 focus:border-blue-900 outline-none" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            {(['Frente', 'Verso'] as const).map((side, si) => {
              ...
              return (
                <div key={side}>
                  <p className="text-[9px] font-black text-gray-400 uppercase mb-1">{side}</p>
                  {url ? (
                    <div className="relative">
                      <img src={url} className="w-full h-20 object-cover rounded-xl border-2 border-blue-900" alt={side} />
                    ...
                    <button type="button" onClick={() => pickPhoto(setUrl)}
                      className="w-full h-20 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-1 hover:border-blue-900 hover:bg-blue-50 transition-all">
