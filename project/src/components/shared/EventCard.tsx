...
export function EventCard({ ev, clientId, clientName, clientPhone }: Props) {
  ...
  return (
    <>
      <div className="bg-white border-2 border-gray-100 rounded-[22px] overflow-hidden hover:border-blue-200 transition-all group">
        ...
        <div className="relative">
          {photos.length > 0 ? (
            ...
            <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-5 py-4 flex items-start gap-4">
              <div className="w-12 h-12 bg-white/20 rounded-[14px] flex items-center justify-center flex-shrink-0 text-2xl">
                {emoji}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border ${tipoCls}`}>{ev.tipo}</span>
                  {urgencyBadge && (
                    <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${urgencyBadge.cls}`}>{urgencyBadge.label}</span>
                  )}
                </div>
                <p className="font-black text-white text-sm leading-tight line-clamp-2">{ev.title}</p>
                <p className="text-blue-300 text-[11px] font-bold mt-0.5 truncate">{ev.company_name}</p>
              </div>
            </div>
          )}
          ...
          {photos.length > 0 && (
            <div className="absolute top-5 left-5 flex flex-wrap gap-1.5 pointer-events-none">
              <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border backdrop-blur-sm bg-white/80 ${tipoCls}`}>{ev.tipo}</span>
              {urgencyBadge && (
                <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${urgencyBadge.cls} backdrop-blur-sm`}>{urgencyBadge.label}</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          {photos.length > 0 && (
            <div>
              <p className="font-black text-blue-900 text-sm leading-tight">{ev.title}</p>
          ...
          {ev.description && (
            <>
              ...
              {expanded && (
                <p className="text-xs font-bold text-gray-600 leading-relaxed bg-gray-50 rounded-[12px] px-3 py-2.5">
                  {ev.description}
                </p>
              )}
            </>
          ...
          <button
            ...
            className="w-full bg-blue-900 hover:bg-blue-800 active:scale-[0.98] text-white py-3.5 rounded-[14px] font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-md"
          >
