...
const STATUS_MAP: Record<string, { label: string; cls: string; dot: string }> = {
  ...
  awaiting_guests: { label: '👥 Aguardando convidados',  cls: 'bg-blue-100 text-blue-700 border-blue-200',   dot: 'bg-blue-400'  },
...
function GuestModal({ booking, tripData, onClose, onSaved }: GuestModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-4 bg-blue-900/60 backdrop-blur-md">
      <div className="bg-white w-full max-w-md max-h-[90vh] flex flex-col rounded-[32px] shadow-2xl border-4 border-blue-900 animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">

        <div className="flex-shrink-0 bg-blue-900 px-6 pt-5 pb-4 rounded-t-[28px]">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h2 className="text-lg font-black text-white uppercase italic">Convidados</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          {errors.length > 0 && (
            <div className="bg-red-50 border-2 border-red-200 rounded-[14px] px-4 py-3 space-y-1">
              {errors.map((e, i) => (
          ...
          <p className="text-xs font-bold text-gray-400 leading-relaxed">
            ...
            <span className="text-blue-900 font-black">email cadastrado</span>.
            Pode confirmar sem preencher todos.
          </p>

          {guests.length === 0 && (
            <div className="py-6 text-center bg-gray-50 rounded-[16px] border-2 border-dashed border-gray-200">
              <p className="text-sm font-black text-gray-300 uppercase italic">Nenhum convidado adicionado</p>
              <p className="text-xs text-gray-300 font-bold mt-1">Pode confirmar assim ou adicionar abaixo</p>
            </div>
          )}

          {guests.map((g, idx) => (
            <div key={idx} className={'border-2 rounded-[16px] p-4 space-y-3 transition-all ' + (g.client_id ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white')}>
              <div className="flex items-center justify-between">
                <p className="text-[10px] font-black text-gray-500 uppercase">Convidado {idx + 1}</p>
              ...
              <div>
                <p className="text-[9px] font-black text-gray-400 uppercase mb-1">Email cadastrado</p>
                <input
                  type="email"
                  placeholder="email@exemplo.com"
                  value={g.email}
                  onChange={e => lookupByEmail(idx, e.target.value)}
                  className={'w-full border-2 rounded-[10px] px-3 py-2.5 font-bold text-blue-900 text-sm outline-none transition-all ' +
                    (g.client_id ? 'bg-white border-green-300 focus:border-green-400' : 'bg-white border-gray-200 focus:border-blue-900')}
                />
              </div>
              ...
              {(g.client_id || g.email) && (
                <div>
                  ...
                  <input
                    ...
                    className={'w-full border-2 rounded-[10px] px-3 py-2.5 font-bold text-blue-900 text-sm outline-none transition-all ' +
                      (g.client_id ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200 focus:border-blue-900')}
                  />
                </div>
              )}

              {g.client_id && g.profile_number && (
                <div className="flex items-center gap-2 bg-white border border-green-200 rounded-[10px] px-3 py-2">
              ...
              {g.email && !g.client_id && g.email.includes('@') && (
                <div className="bg-red-50 border border-red-200 rounded-[10px] px-3 py-2 flex items-center gap-2">
          ...
          {guests.length < maxGuests && (
            <button onClick={addGuest}
              className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-blue-200 hover:border-blue-900 text-blue-600 hover:text-blue-900 rounded-[16px] py-3 font-black text-sm uppercase transition-all hover:bg-blue-50">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <p className="text-[10px] font-black text-gray-500 uppercase">Observação (opcional)</p>
            </div>
            <textarea
              value={obs}
              rows={2}
              ...
              className="w-full bg-gray-50 border-2 border-gray-100 focus:border-blue-900 rounded-[14px] px-4 py-3 font-bold text-blue-900 text-sm outline-none transition-all resize-none placeholder:text-gray-300"
            />
          </div>
        </div>
        ...
        <div className="flex-shrink-0 border-t-2 border-gray-100 px-5 py-4 flex gap-3">
          <button onClick={onClose}
            className="px-5 py-3.5 border-2 border-gray-100 text-gray-400 rounded-[20px] font-black text-sm uppercase hover:border-gray-300 transition-all">
            Cancelar
          </button>
          <button onClick={handleSave} disabled={hasUnregisteredGuests}
            ...
            className={'flex-1 py-3.5 rounded-[20px] font-black uppercase text-sm shadow-lg transition-all flex items-center justify-center gap-2 ' +
              (saved ? 'bg-green-500 text-white' : hasUnregisteredGuests ? 'bg-gray-200 text-gray-400 cursor-not-allowed shadow-none' : 'bg-blue-900 text-white hover:bg-blue-800')}>
...
export function BookingCard({ b, onRefresh }: { b: Booking; onRefresh?: () => void }) {
  ...
  return (
    <div>
      <div className={'bg-white rounded-[20px] border-2 overflow-hidden transition-all ' + (open ? 'border-blue-900 shadow-md' : 'border-gray-100')}>
        ...
        <button type="button" onClick={() => setOpen(v => !v)} className="w-full flex items-stretch text-left hover:bg-gray-50 transition-colors">
          ...
          <div className="flex-1 px-4 py-3 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <p className="font-black text-blue-900 uppercase italic truncate text-sm">{tripData?.boat_name || 'Passeio'}</p>
              {b.booking_number && (
                <span className="text-[9px] font-black text-blue-400 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded-full flex-shrink-0 flex items-center gap-0.5">
            ...
            <span className={'inline-block mt-1 text-[9px] font-black uppercase px-2 py-0.5 rounded-full border ' + st.cls}>{st.label}</span>
          </div>
          <div className="flex items-center pr-4 flex-shrink-0">
            <ChevronDown className={'w-4 h-4 text-gray-400 transition-transform duration-200 ' + (open ? 'rotate-180 text-blue-900' : '')} />
          </div>
        </button>
        ...
        {canAddGuests && (
          <div className="px-4 pb-3 pt-1 border-t border-blue-50">
            <button
              ...
              className={'w-full flex items-center justify-between px-4 py-2.5 rounded-[14px] border-2 transition-all font-black text-xs uppercase ' +
                (guestsFilled > 0
                  ? 'bg-green-50 border-green-200 text-green-700 hover:border-green-400'
                  : 'bg-blue-50 border-blue-200 text-blue-700 hover:border-blue-900')}
            >
              <div className="flex items-center gap-2">
        ...
        {open && (
          <div className="border-t-2 border-blue-50 bg-gray-50/40 px-5 py-5 space-y-5 animate-in fade-in slide-in-from-top-2 duration-200">
            {b.booking_number && (
              <div className="bg-blue-900 rounded-[18px] px-5 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Ticket className="w-5 h-5 text-blue-300" />
                  <div>
                    <p className="text-blue-400 text-[9px] font-black uppercase tracking-widest">Nº do Bilhete</p>
                    <p className="text-white font-black text-2xl">{fmtTicket(b.booking_number)}</p>
                  </div>
                </div>
                <span className={'text-[10px] font-black px-3 py-1.5 rounded-full border-2 ' + st.cls}>{st.label}</span>
              </div>
            )}
            {photo && (
              <div className="w-full h-40 rounded-[16px] overflow-hidden">
                <img src={photo} alt="" className="w-full h-full object-cover" />
              </div>
            )}
            <div>
              <h3 className="font-black text-blue-900 text-lg uppercase italic">{tripData?.boat_name || 'Passeio'}</h3>
            ...
            {tripData && (
              <div className="bg-blue-900 rounded-[16px] px-4 py-3">
                <p className="text-blue-400 text-[9px] font-black uppercase tracking-widest mb-1.5">Rota</p>
                <div className="flex items-center gap-2">
                  <p className="font-black text-white text-sm uppercase italic truncate flex-1">{tripData.marina_saida}</p>
                  <ChevronRight className="w-4 h-4 text-blue-400 flex-shrink-0" />
                  <p className="font-black text-blue-200 text-sm uppercase italic truncate flex-1">{tripData.marina_chegada}</p>
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                <div key={l} className="bg-white rounded-[12px] px-3 py-2.5 border border-gray-100">
                  <p className="text-[9px] font-black text-gray-400 uppercase">{l}</p>
                  <p className="font-black text-blue-900 text-xs mt-0.5">{v}</p>
                </div>
              ))}
            </div>
            {(b.guests?.length || 0) > 0 && (
              <div>
                ...
                <div className="space-y-1.5">
                  {b.guests!.map((g, i) => (
                    <div key={i} className={'flex items-center gap-3 px-3 py-2.5 rounded-[12px] border-2 ' + (g.client_id ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200')}>
                      <div className={'w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ' + (g.client_id ? 'bg-green-500 text-white' : 'bg-gray-300 text-white')}>
                        {g.name ? g.name[0].toUpperCase() : (i + 1)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-black text-sm text-blue-900 truncate">{g.name || '—'}</p>
            ...
            {b.guests_obs && (
              <div className="bg-amber-50 border border-amber-200 rounded-[14px] px-4 py-3 flex items-start gap-2">
                <MessageSquare className="w-3.5 h-3.5 text-amber-600 flex-shrink-0 mt-0.5" />
                <p className="text-xs font-bold text-amber-800 leading-relaxed">{b.guests_obs}</p>
              </div>
            )}
            {tripData?.minimo_tripulantes > 1 && (
              <div className="bg-amber-50 border border-amber-200 rounded-[14px] px-4 py-2.5 flex items-center gap-2">
                <span>⚠️</span>
                <p className="text-xs font-black text-amber-700">Mínimo de {tripData.minimo_tripulantes} confirmados para realizar</p>
              </div>
            )}
            {(bebidasLabel || comidaLabel || barLabel) && (
              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">🎁 Incluído</p>
                <div className="flex flex-wrap gap-1.5">
                  {[bebidasLabel, comidaLabel, barLabel].filter(Boolean).map((label, i) => (
                    <span key={i} className="text-[10px] font-black text-gray-600 bg-white border border-gray-200 px-2.5 py-1 rounded-full">{label}</span>
                  ))}
                </div>
              </div>
            )}
            {tripData?.descricao && (
              <p className="text-xs font-bold text-gray-600 leading-relaxed bg-white border border-gray-100 rounded-[14px] px-4 py-3">{tripData.descricao}</p>
            )}
            {tripData?.meeting_point && (
              <div className="bg-blue-50 border-2 border-blue-200 rounded-[18px] p-4 space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-black text-blue-900 text-sm">{tripData.meeting_point}</p>
                    {(tripData.meeting_sector || tripData.meeting_gate) && (
                      <p className="text-xs text-blue-600 font-bold mt-0.5">{[tripData.meeting_sector, tripData.meeting_gate].filter(Boolean).join(' · ')}</p>
                    )}
                ...
                {(tripData.meeting_sector || tripData.meeting_gate) && (
                  <div className="grid grid-cols-2 gap-2">
                    {tripData.meeting_sector && (
                      <div className="bg-white rounded-[12px] px-3 py-2.5 border border-blue-100 flex items-center gap-2">
                        <Building2 className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                        <div>
                          <p className="text-[9px] font-black text-gray-400 uppercase">Setor</p>
                          <p className="font-black text-blue-900 text-xs">{tripData.meeting_sector}</p>
                        </div>
                      </div>
                    )}
                    {tripData.meeting_gate && (
                      <div className="bg-white rounded-[12px] px-3 py-2.5 border border-blue-100 flex items-center gap-2">
                        <DoorOpen className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" />
                        <div>
                          <p className="text-[9px] font-black text-gray-400 uppercase">Portão</p>
                          <p className="font-black text-blue-900 text-xs">{tripData.meeting_gate}</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {tripData.meeting_maps_url && (
                  <a href={tripData.meeting_maps_url} target="_blank" rel="noreferrer"
                    className="w-full flex items-center justify-center gap-2 bg-blue-900 hover:bg-blue-800 text-white py-3 rounded-[16px] font-black text-xs uppercase transition-all shadow-md">
                    <Map className="w-4 h-4" /> Ver no Google Maps ↗
                  </a>
                )}
              </div>
            )}
            <div className={'rounded-[14px] px-4 py-3 border-2 flex items-center gap-3 ' + st.cls}>
              <div className={'w-2.5 h-2.5 rounded-full flex-shrink-0 ' + st.dot} />
              <p className="font-black text-sm">{st.label}</p>
            </div>
          </div>
        )}
      </div>
