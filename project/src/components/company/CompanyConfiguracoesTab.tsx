// src/components/company/CompanyConfiguracoesTab.tsx
import { useState } from 'react';
import {
  LogOut, KeyRound, Eye, EyeOff, Check, Copy, Trash2,
  AlertCircle, ChevronUp, ChevronDown, Building2, Save,
} from 'lucide-react';
import { updateCompany, deleteCompany, type Company } from '../../lib/localStore';
...
export function CompanyConfiguracoesTab({ company, onLogout, onCompanyChange }: Props) {
  ...
  const senhaDisplay = company.company_password || '—';
  ...
  async function savePass() {
    ...
    await updateCompany(company.id, { company_password: newPass });
    onCompanyChange({ company_password: newPass });
    setSaved(true);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Dados da Empresa */}
      <CollapsibleSection icon={Building2} label="Dados da Empresa" open={showDados} onToggle={() => setShowDados(v => !v)}>
        <DadosEmpresaForm
          company={company}
          onSaved={patch => onCompanyChange(patch)}
        />
      </CollapsibleSection>
...
function DadosEmpresaForm({ company, onSaved }: {
  company: Company; onSaved: (patch: Partial<Company>) => void;
}) {
  ...
  const dirty =
    form.nome_fantasia !== (company.nome_fantasia ?? '') ||
    form.razao_social  !== (company.razao_social  ?? '') ||
    form.setor         !== (company.setor         ?? '') ||
    form.telefone      !== (company.telefone      ?? '') ||
    form.email         !== (company.email         ?? '') ||
    form.website       !== (company.website       ?? '') ||
    form.cidade        !== (company.cidade        ?? '') ||
    form.endereco      !== (company.endereco      ?? '') ||
    form.descricao     !== (company.descricao     ?? '');

  async function handleSave() {
    setError(null);
    if (!form.nome_fantasia.trim()) { setError('O nome é obrigatório.'); return; }
    if (!form.email.trim())          { setError('O email é obrigatório.'); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) { setError('Email inválido.'); return; }

    setSaving(true);
    try {
      await updateCompany(company.id, form as any);
      onSaved(form as any);
      setSuccess(true);
  ...
  const fields: { key: keyof typeof form; label: string; type?: string; placeholder?: string; full?: boolean; textarea?: boolean }[] = [
    { key: 'nome_fantasia', label: 'Nome Fantasia', full: true },
    { key: 'razao_social',  label: 'Razão Social',  full: true },
    { key: 'setor',         label: 'Setor' },
    { key: 'email',         label: 'Email',         type: 'email' },
    { key: 'telefone',      label: 'Telefone',      placeholder: '+351 912345678' },
    { key: 'website',       label: 'Website',       placeholder: 'https://…' },
    { key: 'cidade',        label: 'Cidade' },
    { key: 'endereco',      label: 'Endereço',      full: true },
    { key: 'descricao',     label: 'Descrição',     full: true, textarea: true },
  ];
          ...
          <div key={f.key} className={f.full ? 'sm:col-span-2' : ''}>
            ...
            {f.textarea ? (
              <textarea
                ...
                rows={3}
                className="w-full bg-gray-50 border border-gray-200 py-3 px-3 font-medium text-[#1a2b4a] focus:border-[#c9a96e] outline-none text-sm transition-colors resize-none"
              />
            ) : (
              <input
                type={f.type || 'text'}
