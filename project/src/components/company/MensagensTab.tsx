    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold">Notificações e avisos</p>
        </div>
        {unread > 0 && (
          <span className="bg-red-500 text-white text-xs font-black px-3 py-1.5 rounded-full flex items-center gap-1.5">
      ...
      <div className="space-y-2">
        {displayMessages.length === 0 ? (
          ...
          displayMessages.map(m => {
            ...
            return (
              <div key={m.id}
                className={`border-2 rounded-[16px] overflow-hidden transition-all ${
                  MSG_COLORS[m.type] || 'bg-gray-50 border-gray-100'
                } ${!m.read ? 'ring-2 ring-blue-300' : ''}`}>
                <button
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                      <p className={`text-sm truncate ${!m.read ? 'font-black text-blue-900' : 'font-bold text-gray-700'}`}>{m.title}</p>
                ...
                {isOpen && (
                  <div className="px-4 pb-4 space-y-3 border-t border-black/5">
                    <p className="text-xs font-bold text-gray-600 leading-relaxed pt-3">{m.body}</p>
                    <div className="flex gap-2 pt-1">
                      <button
                        onClick={() => archiveMessage(m.id)}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-[10px] font-black text-[10px] uppercase transition-all">
                        <Inbox className="w-3 h-3" /> Arquivar
                      </button>
                      <button
                        onClick={() => deleteMessage(m.id)}
                        className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-red-50 hover:bg-red-100 border border-red-100 text-red-600 rounded-[10px] font-black text-[10px] uppercase transition-all">
