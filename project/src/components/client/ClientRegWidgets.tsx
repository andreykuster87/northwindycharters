  ...
  return (
    <div ref={ref} className="relative flex-1">
      <button type="button" onClick={() => setOpen(v => !v)}
        className={`w-full bg-gray-50 border-2 rounded-[14px] py-3.5 px-3 font-bold text-sm text-center transition-all outline-none
          ...
          <div className="max-h-48 overflow-y-auto">
            {items.map(item => (
              <button key={item} type="button"
                ...
                className={`w-full px-3 py-2.5 text-center text-sm font-bold transition-colors hover:bg-blue-50
                  ${value === item.split(' ')[0] ? 'text-blue-900 font-black' : 'text-gray-400'}`}>
                {item}
              </button>
            ))}
          </div>
...
export function CountryDropdown({ value, onChange, label }: {
  ...
  return (
    <div className="relative">
      <label className="text-[10px] font-black text-blue-900 uppercase tracking-wider ml-1 mb-1.5 block">{label}</label>
      <button type="button" onClick={() => setOpen(v => !v)}
...
export function DocTypeDropdown({ value, onChange }: {
  ...
  return (
    <div className="relative">
      <label className="text-[10px] font-black text-blue-900 uppercase tracking-wider ml-1 mb-1.5 flex items-center gap-1.5">
        <CreditCard className="w-3 h-3" /> Tipo de Documento *
      </label>
      <button type="button" onClick={() => setOpen(v => !v)}
        className="w-full bg-gray-50 border-2 border-gray-100 rounded-[18px] py-4 px-5 font-bold text-blue-900 outline-none transition-all text-sm flex items-center justify-between hover:border-blue-300">
    ...
    <div>
      <label className="text-[10px] font-black text-blue-900 uppercase tracking-wider ml-1 mb-1.5 flex items-center gap-1.5">
        ...
        {required
          ...
          : <span className="text-gray-300 font-bold normal-case">(opcional)</span>}
      </label>
      {sublabel && <p className="text-[10px] text-gray-400 font-bold ml-1 mb-2">{sublabel}</p>}
