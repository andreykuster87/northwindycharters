  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {FUNCOES_MARITIMAS.map(f => {
            ...
            return (
              <button key={f} type="button"
                ...
                className={`text-left px-3 py-2.5 text-xs font-bold transition-all border ${
                  sel ? 'bg-[#0a1628] text-white border-[#0a1628]' : 'bg-gray-50 text-[#1a2b4a] border-gray-200 hover:border-[#c9a96e]/30'
                }`}>
                {f}
                {sel && <span className="ml-1 text-[#c9a96e]">✓</span>}
              </button>
            );
          })}
        </div>
      ...
      <div className="bg-gray-50 border border-gray-200 p-5 space-y-4">
        <p className="text-[11px] font-semibold text-[#c9a96e] uppercase tracking-[0.15em]">⚓ Caderneta Marítima</p>
