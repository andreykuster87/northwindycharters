    ...
    <div className="space-y-4 animate-in fade-in duration-200">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onAddEntry}
          className="flex items-center gap-1.5 bg-blue-900 hover:bg-blue-800 text-white px-3 py-2 rounded-full font-black text-[10px] uppercase transition-all">
      ...
      <div className="space-y-2.5">
        {schedule
          ...
          .map(entry => {
            ...
            return (
              <div key={entry.id} className={`border-2 rounded-[18px] overflow-hidden transition-all
                ${isPast ? 'border-gray-100 opacity-60' : isOpen ? 'border-blue-900' : 'border-gray-100'}`}>

                {/* Cabeçalho da data */}
                <div onClick={() => setExpandedEntry(isOpen ? null : entry.id)}
                  className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all
                    ${isOpen ? 'bg-blue-900' : 'bg-gray-50 hover:bg-blue-50'}`}>

                  <CalendarDays className={`w-4 h-4 flex-shrink-0 ${isOpen ? 'text-blue-300' : isPast ? 'text-gray-300' : 'text-blue-500'}`} />

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className={`font-black text-sm ${isOpen ? 'text-white' : isPast ? 'text-gray-400' : 'text-blue-900'}`}>
                        {fmt(entry.date)}
                      </p>
                      {isPast && (
                        <span className="text-[9px] font-black bg-gray-200 text-gray-500 px-1.5 py-0.5 rounded-full">passada</span>
                      )}
                    </div>
                    <p className={`text-[10px] font-bold truncate mt-0.5 ${isOpen ? 'text-blue-300' : 'text-gray-400'}`}>
                ...
                {/* Painel expandido */}
                {isOpen && (
                  <div className="p-4 space-y-4 bg-white border-t-2 border-blue-100 animate-in fade-in duration-150">
                      ...
                      {duracao && entry.time_slots.length > 0 && (
                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                          ...
                          <span className="flex items-center gap-1 text-[9px] font-black text-gray-400 uppercase">
                            <span className="w-3 h-3 bg-gray-100 border border-gray-200 rounded-[3px] inline-block" /> Livre
                          </span>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-5 gap-1.5">
                        {TIME_OPTIONS.map(slot => {
                          ...
                          const cls = active
                            ...
                            : 'bg-gray-50 text-gray-500 border-gray-100 hover:border-blue-400 hover:text-blue-900';
                          return (
      ...
      {schedule.length === 0 && (
        <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[18px] py-10 text-center">
          <CalendarDays className="w-8 h-8 text-gray-300 mx-auto mb-2" />
          <p className="text-gray-400 font-black text-xs uppercase">Nenhuma data cadastrada</p>
        </div>
      )}
    </div>
  );
